 ...
 */
@ConfigurationProperties(prefix = "raas.youtube")
@Validated
public record YouTubeProperties(
         ...
         */
        @NotBlank(message = "YouTube API key must not be blank") String apiKey,

        /**
         * YouTube channel ID to fetch data from
         * Must be a valid, non-empty channel ID (typically starts with UC)
         */
        @NotBlank(message = "YouTube channel ID must not be blank") @Pattern(regexp = "^(UC|UU|HC)[a-zA-Z0-9_-]{22}$", message = "YouTube channel ID must be a valid format (24 characters starting with UC, UU, or HC)") String channelId,

        /**
         * Application name to use for YouTube API requests
         * Must be non-blank, default: "raas-youtube-mcp"
         */
        @NotBlank(message = "YouTube application name must not be blank") String applicationName
    ...
    public YouTubeProperties {
        if (applicationName == null || applicationName.trim().isEmpty()) {
            applicationName = "raas-youtube-mcp";
        }
        ...
        if (apiKey != null && (apiKey.length() < 10 || apiKey.length() > 100)) {
            throw new IllegalArgumentException(
                    "YouTube API key length seems invalid. Expected 10-100 characters, got: " + apiKey.length());
        }
    }
